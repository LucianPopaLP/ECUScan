    ...
    with open(filename, "r") as f:                                    # open any files from the selected path
        if (f.name.find("csv") != -1):                                # do the following steps just for .csv files
            ...
            for line in f:                                            # go again through all lines from the file
                if(new_line_count < 2):                               # write metadata lines before voltage data 
                   g.write(line)                                      # write metadata lines before voltage data 
                if(new_line_count == 3):                              # write metadata lines before voltage data 
                   g.write("8\n")                                     # write metadata lines before voltage data 
                if(new_line_count == 4):                              # write metadata lines before voltage data 
                   g.write("Time,Channel C,Channel D\n")              # write metadata lines before voltage data 
                if(new_line_count == 5):                              # write metadata lines before voltage data 
                   g.write("(ms),(V),(V)\n")                          # write metadata lines before voltage data 
                if(new_line_count > align_voltage_index - 240 and new_line_count <= align_voltage_index + 2260):      
                                                                      # check if line index is in the expected range
                   g.write(line.split(",")[0] + "," + line.split(",")[1] + "," + line.split(",")[2] + "\n")           
                                                                      # write 2500 lines - all lines between align_voltage_index - 240 and align_voltage_index + 2260
                                                                      # without (Channel C - Channel D) column
